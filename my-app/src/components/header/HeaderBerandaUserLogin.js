...
function HeaderLogin(props) {
	...
	return (
		<Box sx={{ backgroundColor: 'white', display: 'flex', justifyContent: 'space-between', py: 1, boxShadow: '0px 0px 2px 0px', position: 'fixed', width: '100%', zIndex: '999' }}>
			...
			>
				<Box sx={{ p: '8px 16px 8px 16px' }}>
					<Typography variant='h5'>Rekapitulasi Dana</Typography>
				</Box>
				<Divider />
				<MenuItem><Link to='#' style={{ textDecoration: 'none', color: '#616161' }}>Beasiswa</Link></MenuItem>
				<MenuItem><Link to='#' style={{ textDecoration: 'none', color: '#616161' }}>Non Beasiswa</Link></MenuItem>
			</Menu>
			<Box sx={{ pt: 1, mr: 18 }}>
			...
			<Box >
				<Button variant='outlined' onClick={() => navigate('#')} color='success' sx={{ marginRight: 2 }}>
					Collect Sertificate!
				</Button>
			</Box>
		</Box >
